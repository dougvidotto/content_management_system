...
class CMSDTest < Minitest::Test
  ...
  def test_file_content
    ...
    create_document "changes.txt", content, data_path
...
# This is an example of a file that contains Markdown code!

With the markdown it is easy to format code that is easy to read.
Also I can use this language to put some *emphasis* in italic, or even
**bold**.
...
# Another headline
    ...
    assert_includes last_response.body, "<em>emphasis</em>"
  ...
  def test_post_changes_into_file
    create_document "about.md", "# This is ruby..", data_path
    create_document "hist.yml", "", data_path + "/history"
    ...
    assert_match (/about_\d{4}-\d{1,2}-\d{1,2}_\d{1,2}h-\d{1,2}m-\d{1,2}s.md/), hist_files.first
    assert_equal 'about.md has been updated.', session[:alert]
    
    get '/about.md/edit', {}, admin_session
    
    assert_match (/about_\d{4}-\d{1,2}-\d{1,2}_\d{1,2}h-\d{1,2}m-\d{1,2}s.md/), last_response.body
    assert_includes last_response.body, "Change made by admin"
  ...
  def test_accessing_historic_file_from_wrong_original_file
    create_document "about.md", "# This is ruby..", data_path
    create_document "change.txt", "Text example", data_path
    create_document "hist.yml", "", data_path + "/history"
  ...
  def test_accessing_duplicate_file_page
    create_document "history.txt", "History file content", data_path
  ...
  def test_accessing_image_upload_page
    ...
    assert_equal 200, last_response.status
    assert_includes last_response.body, "<input type='file' name='image' accept='.jpg, .jpeg, .png, .gif'>"
  end
  ...
  def test_uploading_file_that_is_not_an_image
    ...
    assert_equal 422, last_response.status
    assert_includes last_response.body, 'File must have one of these extensions: .jpg, .jpeg, .png or .gif.'
  end
  
  def test_upload_file_successfully
    post '/image/new',
      {"image" => Rack::Test::UploadedFile.new(image_path + "/document.jpg", "image/jpeg")},
      admin_session
